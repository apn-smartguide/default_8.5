...
var dataTablesController = { 
	...
	bindEvents : function(sgRef, context) {
		...
		$('[name=select_all]', 'table.table thead tr th').first().unbind('click').bind('click', function(){
		...
		$('input[type="checkbox"]', 'table.table tbody').unbind('change').bind('change', function(){
			var dataTable = $(this).closest('table').DataTable();
