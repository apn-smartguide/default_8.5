...
<apn:control runat="server" id="control">
	...
	<% } else if ((Context.Items["btn-group"] != null && Convert.ToBoolean(Context.Items["btn-group"])) || (Context.Items["btn-toolbar"] != null && Convert.ToBoolean(Context.Items["btn-toolbar"]))) { %>
		<button type="button" id='<apn:name runat="server"/>' name='<apn:name runat="server"/>' class='<apn:cssclass runat="server"/> <%=Context.Items["readonly"]%>' style='<apn:controlattribute runat="server" attr="style"/> <apn:cssstyle runat="server"/>' <% if (!GetTooltip(control.Current).Equals("")){ %> data-toggle='tooltip' data-html='true' <%=Context.Items["tooltip-attribute"]%> <% } %> <% if (control.Current.getCSSClass().IndexOf("disable-event-targets") == -1) { %> <% if(!control.Current.getAttribute("eventtarget").Equals("")) { %>data-eventtarget='[<%=control.Current.getAttribute("eventtarget")%>]' aria-controls='<%=control.Current.getAttribute("eventtarget").Replace("\"","")%>' <% } %> <% } %> <apn:metadata runat="server" /> <%=Context.Items["aria-label"]%>><apn:value runat="server" /></button>
	<% } else if (Context.Items["no-col"] != null && Convert.ToBoolean(Context.Items["no-col"])) { %>
		<span id='div_<apn:name runat="server" />' <% if(!control.Current.getAttribute("eventsource").Equals("")) { %>aria-live='polite' <% } %>>
			<button type="button" class='<apn:cssclass runat="server"/> <%=Context.Items["readonly"]%>' name='<apn:name runat="server"/>' style='<apn:controlattribute runat="server" attr="style"/> <apn:cssstyle runat="server"/>' <% if (!GetTooltip(control.Current).Equals("")){ %> data-toggle='tooltip' data-html='true' <%=Context.Items["tooltip-attribute"]%> <% } %> <% if (control.Current.getCSSClass().IndexOf("disable-event-targets") == -1) { %> <% if(!control.Current.getAttribute("eventtarget").Equals("")) { %>data-eventtarget='[<%=control.Current.getAttribute("eventtarget")%>]' aria-controls='<%=control.Current.getAttribute("eventtarget").Replace("\"","")%>' <% } %> <% } %> <apn:metadata runat="server" /> <%=Context.Items["aria-label"]%>><apn:value runat="server" /></button>
		</span>	
	<% } else { %>
		<span id='div_<apn:name runat="server" />' <% if(!control.Current.getAttribute("eventsource").Equals("")) { %>aria-live='polite' <% } %>>
			<button type="button" class='<apn:cssclass runat="server"/> <%=Context.Items["readonly"]%>' <%=Context.Items["readonly"]%> name='<apn:name runat="server"/>' style='<apn:controlattribute runat="server" attr="style"/> <apn:cssstyle runat="server"/>' <% if (!GetTooltip(control.Current).Equals("")){ %> data-toggle='tooltip' data-html='true' <%=Context.Items["tooltip-attribute"]%> <% } %> <% if (control.Current.getCSSClass().IndexOf("disable-event-targets") == -1) { %> <% if(!control.Current.getAttribute("eventtarget").Equals("")) { %>data-eventtarget='[<%=control.Current.getAttribute("eventtarget")%>]' aria-controls='<%=control.Current.getAttribute("eventtarget").Replace("\"","")%>' <% } %> <% } %> <apn:metadata runat="server" /> <%=Context.Items["aria-label"]%>><apn:value runat="server" /></button>
		</span>
	<% } %>
	<% Context.Items["tooltip-attribute"] = ""; %>
</apn:control>