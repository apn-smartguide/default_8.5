...
<apn:control runat="server" id="control">
	...
	<% } else if (control.Current.getAttribute("class").Equals("view-xml-button") || control.Current.getAttribute("class").Equals("pdf-button")) { %>
		<span class="ml-1 mr-1" id='div_<apn:name runat="server" />' <% if(!control.Current.getAttribute("eventsource").Equals("")) { %>aria-live='polite' <% } %>>
			...
			<%-- <button type='submit' name='<apn:name runat="server"/>' value='<apn:value runat="server"/>' class='<apn:cssclass runat="server"/>' style='<apn:controlattribute runat="server" attr="style"/> <apn:cssstyle runat="server"/>' <% if (!GetTooltip(control.Current).Equals("")){ %> data-toggle='tooltip' data-html='true' <%=Context.Items["tooltip-attribute"]%> <% } %> onclick='document.forms[/'pdf/'].elements[/'pdf/'].value=/'<apn:name runat="server"/>/'; document.forms[/'pdf/'].submit(); return false;' <%=Context.Items["aria-label"]%> /> --%>
				<% if(control.Current.getCSSClass().Contains("safe-pdf")) { %> <%-- must use this if CSRF enabled --%>
					<a href='genpdf/do.aspx/view.pdf?cache=<%= System.Guid.NewGuid().ToString() %>&pdf=<apn:name runat="server"/>&interviewID=<apn:control type="interview-code" runat="server"><apn:value runat="server"/></apn:control>' target='_blank' <% if (!GetTooltip(control.Current).Equals("")){ %> data-toggle='tooltip' data-html='true' <%=Context.Items["tooltip-attribute"]%> <% } %> class='btn <apn:cssclass runat="server"/>' style='<apn:controlattribute runat="server" attr="style"/> <apn:cssstyle runat="server"/>' <%=Context.Items["aria-label"]%>><apn:value runat="server"/></a>
				<% } else { %>
					<a href='genpdf/do.aspx?t_<%=control.Current.getFieldId()%>=t_<%=control.Current.getFieldId()%>&cache=<%= System.Guid.NewGuid().ToString() %>&pdf=<apn:name runat="server"/>&id=<apn:code runat="server"/>' target='_blank' <% if (!GetTooltip(control.Current).Equals("")){ %> data-toggle='tooltip' data-html='true' <%=Context.Items["tooltip-attribute"]%> <% } %> class='<apn:cssclass runat="server"/>' style='<apn:controlattribute runat="server" attr="style"/> <apn:cssstyle runat="server"/>' <%=Context.Items["aria-label"]%>><apn:value runat="server"/></a>
				<% } %>
			<% } %>
			...
			<%-- <button type='submit' name='<apn:name runat="server"/>' class='<apn:cssclass runat="server"/>' style='<apn:controlattribute runat="server" attr="style"/> <apn:cssstyle runat="server"/>' <% if (!GetTooltip(control.Current).Equals("")){ %> data-toggle='tooltip' data-html='true' <%=Context.Items["tooltip-attribute"]%> <% } %> onclick='document.forms[/'genXML/'].elements[/'xsd/'].value=/'<apn:name runat="server"/>/'; document.forms[/'genXML/'].submit(); return false;' <%=Context.Items["aria-label"]%>><apn:value runat="server"/></button> --%>
	...
	<% } else if ((Context.Items["btn-group"] != null && Convert.ToBoolean(Context.Items["btn-group"])) || (Context.Items["btn-toolbar"] != null && Convert.ToBoolean(Context.Items["btn-toolbar"]))) { %>
		<button id='<apn:name runat="server"/>' name='<apn:name runat="server"/>' class='<apn:cssclass runat="server"/> <%=Context.Items["readonly"]%>' style='<apn:controlattribute runat="server" attr="style"/> <apn:cssstyle runat="server"/>' <% if (!GetTooltip(control.Current).Equals("")){ %> data-toggle='tooltip' data-html='true' <%=Context.Items["tooltip-attribute"]%> <% } %> <% if(!control.Current.getAttribute("eventtarget").Equals("")) { %>data-eventtarget='[<%=control.Current.getAttribute("eventtarget")%>]' aria-controls='<%=control.Current.getAttribute("eventtarget").Replace("\"","")%>' <% } %> <apn:metadata runat="server" /> <%=Context.Items["aria-label"]%>><apn:value runat="server" /></button>
	<% } else if (Context.Items["no-col"] != null && Convert.ToBoolean(Context.Items["no-col"])) { %>
		<span class="ml-1 mr-1" id='div_<apn:name runat="server" />' <% if(!control.Current.getAttribute("eventsource").Equals("")) { %>aria-live='polite' <% } %>><button class='<apn:cssclass runat="server"/> <%=Context.Items["readonly"]%>' name='<apn:name runat="server"/>' style='<apn:controlattribute runat="server" attr="style"/> <apn:cssstyle runat="server"/>' <% if (!GetTooltip(control.Current).Equals("")){ %> data-toggle='tooltip' data-html='true' <%=Context.Items["tooltip-attribute"]%> <% } %> <% if(!control.Current.getAttribute("eventtarget").Equals("")) { %>data-eventtarget='[<%=control.Current.getAttribute("eventtarget")%>]' aria-controls='<%=control.Current.getAttribute("eventtarget").Replace("\"","")%>' <% } %> <apn:metadata runat="server" /> <%=Context.Items["aria-label"]%>><apn:value runat="server" /></button></span>	
	<% } else { %>
		<span class="ml-1 mr-1" id='div_<apn:name runat="server" />' <% if(!control.Current.getAttribute("eventsource").Equals("")) { %>aria-live='polite' <% } %>>
			<button class='<apn:cssclass runat="server"/> <%=Context.Items["readonly"]%>' <%=Context.Items["readonly"]%> name='<apn:name runat="server"/>' style='<apn:controlattribute runat="server" attr="style"/> <apn:cssstyle runat="server"/>' <% if (!GetTooltip(control.Current).Equals("")){ %> data-toggle='tooltip' data-html='true' <%=Context.Items["tooltip-attribute"]%> <% } %> <% if(!control.Current.getAttribute("eventtarget").Equals("")) { %>data-eventtarget='[<%=control.Current.getAttribute("eventtarget")%>]' aria-controls='<%=control.Current.getAttribute("eventtarget").Replace("\"","")%>' <% } %> <apn:metadata runat="server" /> <%=Context.Items["aria-label"]%>><apn:value runat="server" /></button>
		</span>
	<% } %>
	<% Context.Items["tooltip-attribute"] = ""; %>
</apn:control>