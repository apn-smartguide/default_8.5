...
<apn:control runat="server" id="control">
	...
	<% } else if (control.Current.getAttribute("class").Equals("view-xml-button") || control.Current.getAttribute("class").Equals("pdf-button")) { %>
		<span id='div_<apn:name runat="server" />' <% if(!control.Current.getAttribute("eventsource").Equals("")) { %>aria-live='polite' <% } %>>
			...
			<%-- <button type='submit' name='<apn:name runat="server"/>' class='<apn:cssclass runat="server"/>' style='sg <apn:controlattribute runat="server" attr="style"/> <apn:cssstyle runat="server"/>' <% if (!GetTooltip(control.Current).Equals("")){ %> data-toggle='tooltip' data-html='true' <%=Context.Items["tooltip-attribute"]%> <% } %> onclick='document.forms[/'genXML/'].elements[/'xsd/'].value=/'<apn:name runat="server"/>/'; document.forms[/'genXML/'].submit(); return false;' <%=Context.Items["aria-label"]%>><apn:value runat="server"/></button> --%>
