...
<apn:control runat="server" id="control">
	...
	<% } else if ((Context.Items["btn-group"] != null && Convert.ToBoolean(Context.Items["btn-group"])) || (Context.Items["btn-toolbar"] != null && Convert.ToBoolean(Context.Items["btn-toolbar"]))) { %>
		<button id='<apn:name runat="server"/>' name='<apn:name runat="server"/>' <%=Context.Items["tooltip-attribute"]%> class='<apn:cssclass runat="server"/> <%=Context.Items["readonly"]%>' style='<apn:controlattribute runat="server" attr="style"/> <apn:cssstyle runat="server"/>' <% if(!control.Current.getAttribute("eventtarget").Equals("")) { %>data-eventtarget='[<%=control.Current.getAttribute("eventtarget")%>]' aria-controls='<%=control.Current.getAttribute("eventtarget").Replace("\"","")%>' <% } %> <apn:metadata runat="server" />><apn:value runat="server" /></button>
