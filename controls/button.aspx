...
<apn:control runat="server" id="control">
	...
	<span id='div_<apn:name runat="server" />' <% if(!control.Current.getAttribute("eventsource").Equals("")) { %>aria-live='polite' <% } %>>
		...
		<a href='genpdf/do.aspx?t_<%=control.Current.getFieldId()%>=t_<%=control.Current.getFieldId()%>&cache=<%= System.Guid.NewGuid().ToString() %>&pdf=<apn:name runat="server"/>&id=<apn:code runat="server"/>' target="_blank" title="<apn:tooltip runat='server'/>" class="<apn:cssclass runat='server'/>" style="<apn:controlattribute runat='server' attr='style'/> <apn:cssstyle runat='server'/>"><apn:value runat='server'/></a>
		<% } %>
		...
		<a href='genxml/do.aspx?t_<%=control.Current.getFieldId()%>=t_<%=control.Current.getFieldId()%>&cache=<%= System.Guid.NewGuid().ToString() %>&xsd=<apn:name runat="server"/>&id=<apn:code runat="server"/>' target="_blank" title="<apn:tooltip runat='server'/>" class="<apn:cssclass runat='server'/>" style="<apn:controlattribute runat='server' attr='style'/> <apn:cssstyle runat='server'/>" ><apn:value runat='server'/></a>
		<% } %>
	</span>
	<% } %>
</apn:control>