			...
			<apn:forEach id="control2" runat="server">
				...
				<apn:choosecontrol runat="server">
					<apn:whencontrol runat="server" type="optgroup">
						...
						<apn:forEach id="control3" runat="server">
							...
							<div <%= Context.Items["layout"] %>>
								<% if(!control3.Current.getLabel().Equals("")) { %><label id='lbl_<apn:controlattribute runat="server" attr="id"/>_<%=Context.Items["optionIndex"]%>_<%=Context.Items["index"]%>' for='<apn:controlattribute runat="server" attr="id" />_<%=Context.Items["optionIndex"]%>_<%=Context.Items["index"]%>' title='<%=GetAttribute(control3.Current, "title", true)%>'><% } %>
							<% } else { %>
								<% if(!control3.Current.getLabel().Equals("")) { %><label id='lbl_<apn:controlattribute runat="server" attr="id"/>_<%=Context.Items["optionIndex"]%>_<%=Context.Items["index"]%>' for='<apn:controlattribute runat="server" attr="id" />_<%=Context.Items["optionIndex"]%>_<%=Context.Items["index"]%>' class='checkbox-inline' title='<%=GetAttribute(control3.Current, "title", true)%>'><% } %>
							<% } %>
								<input type='checkbox' name='<%= control3.Current.getName() %>' id='<%= control3.Current.getAttribute("id")%>_<%= Context.Items["optionIndex"]%>_<%=Context.Items["index"]%>' class='<%=control.Current.getCSSClass()%>' value='<%= control3.Current.getHTMLValue() %>' <apn:metadata runat="server" /> <% if(!control.Current.getAttribute("eventtarget").Equals("")) { %> data-eventtarget='[<%= control.Current.getAttribute("eventtarget")%>]' aria-controls='<%= control.Current.getAttribute("eventtarget").Replace("\"","")%>' <% } %> <% if(!control3.Current.getLabel().Equals("")) { %>aria-labelledby='lbl_<apn:controlattribute runat="server" attr="id" />_<%=Context.Items["optionIndex"]%>_<%=Context.Items["index"]%>'<% } %> <%= control.Current.containsValue(control3.Current.getValue()) ? " checked='checked'" : "" %> <%= Context.Items["layout"] %> /><% ExecutePath("/controls/custom/control-label.aspx"); %>
					...
					<apn:otherwise runat="server">
						<% if(((string)Context.Items["layout"]).Equals("vertically")) { %>
						<div <%= Context.Items["layout"] %>>
							<% if(!control2.Current.getLabel().Equals("")) { %><label id='lbl_<apn:controlattribute runat="server" attr="id"/>_<%=Context.Items["optionIndex"]%>_<%=Context.Items["index"]%>' for='<apn:controlattribute runat="server" attr="id" />_<%=Context.Items["optionIndex"]%>_<%=Context.Items["index"]%>' title='<%=GetAttribute(control2.Current, "title", true)%>'><% } %>
						<% } else { %>
							<% if(!control2.Current.getLabel().Equals("")) { %><label id='lbl_<apn:controlattribute runat="server" attr="id"/>_<%=Context.Items["optionIndex"]%>_<%=Context.Items["index"]%>' for='<apn:controlattribute runat="server" attr="id" />_<%=Context.Items["optionIndex"]%>_<%=Context.Items["index"]%>' class='checkbox-inline' title='<%=GetAttribute(control2.Current, "title", true)%>'><% } %>
						<% } %>
							<input type='checkbox' name='<%= control2.Current.getName() %>' id='<%= control2.Current.getAttribute("id")%>_<%=Context.Items["optionIndex"]%>_<%=Context.Items["index"]%>' class='<%=control.Current.getCSSClass()%>' <% if(!control2.Current.getLabel().Equals("")) { %> aria-labelledby='lbl_<apn:controlattribute runat="server" attr="id"/>_<%= Context.Items["optionIndex"]%>_<%=Context.Items["index"]%>'<% } %> <% if(!control.Current.getAttribute("eventtarget").Equals("")) { %> data-eventtarget='[<%= control.Current.getAttribute("eventtarget")%>]' aria-controls='<%= control.Current.getAttribute("eventtarget").Replace("\"","")%>'<% } %>  value='<%= control2.Current.getHTMLValue() %>' <apn:metadata runat="server" /> <%= control.Current.containsValue(control2.Current.getValue()) ? " checked='checked'" : "" %> <%= Context.Items["layout"] %> /><% ExecutePath("/controls/custom/control-label.aspx"); %>
							<% if(!control2.Current.getLabel().Equals("")) { %></label><% } %>
						<% if(((string)Context.Items["layout"]).Equals("vertically")) { %></div><% } %>
					</apn:otherwise>
				</apn:choosecontrol>
			</apn:forEach>
			<% if(!ShowErrorsAbove) { %><apn:ifnotcontrolvalid runat="server"><br/><strong id='<apn:name runat="server"/>-error' class='error'><span class="label label-danger"><span class="prefix"><%=Smartlet.getLocalizedResource("theme.text.error-prefix").Replace("{1}", ErrorIndex .ToString()) %></span><%= control.Current.getAlert() %></span></strong></apn:ifnotcontrolvalid><% } %>
			</fieldset>
		</apn:whencontrol>
