...
%>
	<apn:choosecontrol runat="server">
		...
		<div id='div_<apn:name runat="server"/>' class='<% if (Options.Contains("WET")) { %>chkbxrdio-grp <% } %> form-group <% if (Options.Contains("TTS")) { %>tts tts-play<% } %>'>
			...
			<ul <%=Context.Items["readonly"]%> <apn:metadata runat="server" /> class='<% if(((string)Context.Items["layout"]).Equals("horizontally")) { %>checkbox-inline<% } %> <%=Context.Items["no-col-layout"]%> <apn:cssclass runat="server" /> <apn:ifnotcontrolvalid runat="server">has-error</apn:ifnotcontrolvalid>' style='<apn:cssstyle runat="server"/>' <!-- #include file="aria-live.inc" -->>
				...
				<li class="checkbox">
					<apn:choosecontrol runat="server">
						...
						<apn:otherwise runat="server">
							...
							<label class="<% if(LayoutEngine == "BS4") { Response.Output.Write("form-check"); } else { Response.Output.Write("form-check-label"); } %>" id='<%=Context.Items["aria-labelledby"]%>' for='<%=Context.Items["id"]%>' title='<%=(!GetTooltip(control.Current).Equals("")) ? GetTooltip(control.Current) : Context.Items["label"]%>'><% Execute("/controls/custom/control-label.aspx"); %>
							<% if (control.Current.getLabel().Equals("") && control.Current.isRequired()) { %><span class="required" data-toggle='tooltip' data-html='true' title='<apn:localize runat="server" key="theme.text.required"/>'>*</span><% } %>
							</label>
							<% if (control.Current.getCSSClass().Contains("inline")) { %></div><% } %>
						</apn:otherwise>
