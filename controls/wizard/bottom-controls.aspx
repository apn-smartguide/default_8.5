	...
	<div class="col-12">
		<div class="btn-toolbar navigation" role="toolbar">
			...
			if(previousBtn != null && previousBtn.isAvailable()) { %>
				<button type='button' name='<%=previousBtn.getHtmlName()%>' class='sg <%=previousBtn.getCSSClass()%>' style='<%=previousBtn.getCSSStyle()%>' data-eventtarget='[<%=previousEventTargets%>]' <% if (!GetTooltip(previousBtn).Equals("")){ %>title='<%=GetTooltip(previousBtn)%>' aria-label='<%=GetTooltip(previousBtn)%>'<% } %>><%=previousBtn.getLabel()%></button>
			<% } else { %>
			...
			<button type='button' name='<%=nextBtn.getHtmlName()%>' class='sg <%=nextBtn.getCSSClass()%>' style='<%=nextBtn.getCSSStyle()%>' data-eventtarget='[<%=nextEventTargets%>]' <% if (!GetTooltip(nextBtn).Equals("")){ %>title='<%=GetTooltip(nextBtn)%>' aria-label='<%=GetTooltip(nextBtn)%>'<% } %>><%=nextBtn.getLabel()%></button>
			<% } else { %>
