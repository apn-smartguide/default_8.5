...
<div class="row">
	<div class="col-12">
		<div class="btn-toolbar" role="toolbar">
			...
			<% } %>
			<apn:control type="return-cancel" runat="server" id="cancel"><button type='submit' name='<apn:name runat="server"/>' class='sg btn <% if(BoostrapVersion == "4") { Response.Output.Write("btn-secondary"); } else { Reponse.Output.Write("btn-default"); } %> pull-right' <% if (!GetTooltip(cancel.Current).Equals("")){ %>title='<%=GetTooltip(cancel.Current)%>' aria-label='<%=GetTooltip(cancel.Current)%>'<% } %>><%=GetAttribute(cancel.Current, "label")%></button></apn:control>
			...
			<% if (!CurrentPageCSS.Contains("hide-return-to-summary")) { %>
				<apn:control type="summary" runat="server" id="summary"><button type='submit' name='<apn:name runat="server"/>' class='sg btn <% if(BoostrapVersion == "4") { Response.Output.Write("btn-secondary"); } else { Reponse.Output.Write("btn-default"); } %> pull-right' <% if (!GetTooltip(summary.Current).Equals("")){ %>title='<%=GetTooltip(summary.Current)%>' aria-label='<%=GetTooltip(summary.Current)%>'<% } %>><%=GetAttribute(summary.Current, "label")%></button></apn:control>
			<% } %>
