...
<div id='div_<apn:name runat="server"/>' <% if(!control.Current.getAttribute("eventtarget").Equals("")) { %> data-eventtarget='[<%=control.Current.getAttribute("eventtarget") %>]'<% } %> class='<% if(BootstrapVersion == "4") { Response.Output.Write("card"); } else { Response.Output.Write("panel panel-default"); } %> repeat uploads-render <% if ((bool)Context.Items["isSelectable"]) { %> selectable<% } %> <% if (TTSEnabled) { %>tts tts-play<% } %>' style='<%=control.Current.getCSSStyle()%>' <% if(!control.Current.getAttribute("eventsource").Equals("")) { %>aria-live='polite'<% } %> >
	...
	<div class='<% if(BootstrapVersion == "4") { Response.Output.Write("card-header"); } else { Response.Output.Write("panel-heading"); } %> clearfix'>
