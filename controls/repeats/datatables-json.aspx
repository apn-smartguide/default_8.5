		...
		<% if (i>0) Response.Write(","); %>
		{
			...
			for(int j=0;j<fields.Length;j++) {
				...
				if (!fields[j].isAvailable() || fields[j].getCSSClass().Contains("proxy") ) {
					...
					value = "<button id='d_"+fieldid+"["+id+"]' " + tooltipStr + " type='button' class='sg " + fields[j].getCSSClass() + "' style='" + fields[j].getCSSStyle() + "' target='" + fields[j].getNonLocalizedMetaData("target") + "' name='d_"+fieldid+"["+id+"]'>"+label+"</button>";
					...
					for(int k=0; k<grpFields.Length;k++){
						...
						if(grpFields[k].isAvailable()) {
							grpValue = grpValue + "<button id='d_"+ grpFields[k].getId()+"["+id+"]' "+ tooltipStr +" type='button' class='sg " + grpFields[k].getCSSClass() + "' style='" + grpFields[k].getCSSStyle() + "' target='" + grpFields[k].getNonLocalizedMetaData("target") + "' name='d_"+grpFields[k].getId()+"["+id+"]'>"+grpFields[k].getLabel()+"</button>";
						} else {
