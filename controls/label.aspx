...
<apn:control runat="server" id="control">
	<% if (!control.Current.getLabel().Equals("")) { %>
		...
		<label id='lbl_<apn:name runat="server"/>' for='<apn:name runat="server"/>' class='<%if (control.Current.getCSSClass().Contains("inline")) {%> inline <% } %> <apn:ifcontrolrequired runat="server">required</apn:ifcontrolrequired>'><% Execute("/controls/custom/control-label.aspx"); %><apn:ifnotcontrolvalid runat="server"><apn:ifcontrolrequired runat="server"><strong class='required'><%=Smartlet.getLocalizedResource("theme.text.required-suffix")%></strong></apn:ifcontrolrequired></apn:ifnotcontrolvalid></label>
		<% } else { %>
