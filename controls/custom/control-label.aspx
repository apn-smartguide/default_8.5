...
<apn:control runat="server" id="control">
	...
	<apn:ifnotcontrolattribute attr="tooltip" runat="server">
		<span <% if (TTSEnabled && !control.Current.getFieldId().Equals("")) { Response.Output.Write("data-tts='{0}_label'",control.Current.getFieldId()); } %> class='field-name <apn:ifnotcontrolvalid runat="server"></span>has-error</apn:ifnotcontrolvalid> <%= GetCleanCSSClass(control).Replace("datatables", "")%>'><%=Context.Items["label"] + GetMetaDataValue(control.Current, "label-suffix") %></span>
	</apn:ifnotcontrolattribute>
	<apn:ifcontrolattribute attr="tooltip" runat="server">
		<span <% if (TTSEnabled && !control.Current.getFieldId().Equals("")) { Response.Output.Write("data-tts='{0}_label'",control.Current.getFieldId()); } %> class='field-name <apn:ifnotcontrolvalid runat="server"></span>has-error</apn:ifnotcontrolvalid> <%= GetCleanCSSClass(control).Replace("datatables", "")%>' data-toggle='tooltip' data-placement='auto' data-container='body' data-html='true' title='<%=GetTooltip(control.Current)%>'><%=Context.Items["label"] + GetMetaDataValue(control.Current, "label-suffix")%></span>
